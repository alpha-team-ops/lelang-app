...
interface OrganizationSetupResponse {
  ...
  accessToken: string;       // JWT token for immediate authentication
...
interface CheckSetupStatusResponse {
  needsSetup: boolean;       // True if user needs to setup organization
  organizationCode?: string; // User's organization code (if exists)
  role?: string;             // User's role in organization (if exists)
}
```
...
**Note on Token Response:**
The response includes a new JWT token with the `organizationCode` embedded. This allows the frontend to:
1. Store the token immediately
2. Update user authentication context
3. Redirect to dashboard without requiring a re-login
The token contains all necessary claims including the newly assigned `organizationCode` and `role: ADMIN`.
...
**Note on Token Response:**
Same as Create Organization - new token is returned with `organizationCode` and `role` embedded for immediate authentication.
...
### 3. Check Organization Setup Status
**Endpoint:** `GET /api/v1/organization/check-setup`
...
**Response (200 OK) - User needs setup:**
```json
{
  "success": true,
  "data": {
...
**Response (200 OK) - User already has organization:**
```json
{
  "success": true,
  "data": {
...
2. Get current user's organizationCode from database
3. Determine setup status:
   - If user.organizationCode is null/empty → needsSetup = true
   - If user.organizationCode exists → needsSetup = false
4. Return user's role only if has organization
5. Response helps frontend decide routing:
   - needsSetup: true → Redirect to `/auth/organization-setup`
   - needsSetup: false → Allow access to `/admin` dashboard

**Permissions Required:**
- Must be authenticated (any authenticated user can check)

---

### Database Changes Required (All Endpoints)
- Update `users` table: set organizationCode where id = current_user_id
- Insert into audit log
- Optionally: insert into notifications table
