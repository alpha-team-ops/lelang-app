...
const CreateAuctionPage: React.FC = () => {
  ...
  const handleSaveDraft = () => {
    if (validateForm()) {
      ...
      setSubmitted(true);
      setTimeout(() => {
        alert('Lelang berhasil disimpan sebagai draft!');
  ...
  const handlePublish = () => {
    if (validateForm()) {
      ...
      setSubmitted(true);
      setTimeout(() => {
        alert('Lelang berhasil dipublikasikan!');
        navigate('/admin/auctions');
      }, 1000);
    }
  };
  ...
  return (
    <Box sx={{ py: { xs: 2, md: 3 }, px: { xs: 2, md: 3 }, maxWidth: '600px', mx: 'auto' }}>
      {/* Header */}
      ...
      {submitted && (
        <Box sx={{ mt: 2 }}>
          <Alert severity="success">
            ✓ Memproses permintaan Anda... Akan dialihkan dalam beberapa saat.
          </Alert>
        </Box>
      )}
    </Box>
